...
import { ArrowDownRight, ArrowUpRight, Building2, CheckCircle2, ChevronRight, Layers, MapPin, Sparkles } from 'lucide-react';
...
export function Hero() {
  ...
  return (
    <section id="inicio" className="relative min-h-[90vh] overflow-hidden bg-void text-white flex flex-col justify-between">
      <div className="enterprise-grid absolute inset-0 opacity-60" aria-hidden="true" />
      <div className="glow-spot -right-24 top-12 h-96 w-96 bg-accent/15" aria-hidden="true" />
      <div className="glow-spot left-1/4 bottom-10 h-80 w-80 bg-violet/10" aria-hidden="true" />

      <div className="relative mx-auto w-full max-w-[1320px] px-5 pt-24 pb-8 md:px-8 lg:pt-28 flex-1 flex flex-col justify-center">
        <div className="grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:gap-14">
          
          {/* Left Column: Executive Value Proposition */}
          <div className="relative z-10 py-2">
            <div className="mb-5 flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-[10px] uppercase tracking-wider text-slate-400">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-0.5 text-emerald-400">
                <span className="pulse-signal h-1.5 w-1.5 rounded-full bg-emerald-400" />
            ...
            <h1 className="text-balance max-w-[780px] font-display text-[clamp(2.2rem,4.4vw,4.2rem)] font-bold leading-[1.04] tracking-tight text-white">
            ...
            <p className="mt-5 max-w-[660px] text-sm leading-relaxed text-slate-300 md:text-base md:leading-7">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <a
                href="#proyectos"
                className="inline-flex items-center justify-center gap-2 rounded-lg bg-accent px-5 py-3 text-xs font-bold text-white shadow-md transition-all hover:bg-accent-light hover:shadow-lg hover:-translate-y-0.5"
              >
                Explorar Proyectos & Casos <ArrowDownRight className="h-4 w-4" />
              </a>
              <a
                href="#experiencia"
                className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-800/60 px-5 py-3 text-xs font-semibold text-slate-200 transition-colors hover:border-slate-600 hover:bg-slate-700 hover:text-white"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-slate-700/60 bg-transparent px-4 py-3 text-xs font-medium text-slate-400 transition-colors hover:border-slate-600 hover:text-slate-200"
              >
          ...
          <div className="relative mx-auto w-full max-w-[540px] lg:ml-auto">
            <div className="window-shadow-dark relative rounded-2xl border border-slate-800 bg-night">
              
              {/* Card Header */}
              <div className="flex h-11 items-center justify-between border-b border-slate-800/80 bg-slate-900/70 px-4">
              ...
              <div className="p-5 sm:p-6 space-y-3.5">
                ...
                {/* 2. Pregunta Estructural */}
                <div className="rounded-xl border border-accent/25 bg-accent/[0.06] p-3.5">
                  <div className="flex items-center justify-between text-accent-light">
                ...
                {/* Tab Switcher */}
                <div className="pt-2 flex items-center justify-between border-t border-slate-800/80" role="tablist" aria-label="Casos de transformación">
        ...
        <div className="mt-10 grid grid-cols-1 border-t border-slate-800/80 pt-6 sm:grid-cols-3 gap-6">
          {heroSignals.map((signal, index) => (
            <div
              key={signal.value}
              className={`flex flex-col justify-between ${
                index > 0 ? 'sm:border-l sm:border-slate-800 sm:pl-6' : ''
              }`}
            >
              <div className="flex items-baseline gap-2">
                ...
                <span className="font-mono text-[10px] font-semibold uppercase tracking-wider text-accent-light">
                  {signal.label}
                </span>
              </div>
