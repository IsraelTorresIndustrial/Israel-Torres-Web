...
export function Footer() {
  ...
  return (
    <footer className="border-t border-white/10 bg-void py-9 text-white">
      <div className="mx-auto flex max-w-[1320px] flex-col gap-6 px-5 md:flex-row md:items-center md:justify-between md:px-8">
        <div className="flex items-center gap-4">
          <span className="relative grid h-10 w-10 place-items-center overflow-hidden rounded-xl border border-white/15 font-display text-xs font-semibold">
          ...
          <div>
            <p className="font-display text-sm font-semibold">{personalInfo.fullName}</p>
            <p className="mt-1 font-mono text-[9px] uppercase tracking-[0.13em] text-white/40">
              Strategy, Product & Applied AI · {new Date().getFullYear()}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <a
            href="#inicio"
            className="ml-2 inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2.5 font-mono text-[9px] uppercase tracking-wider text-white/50 transition-colors hover:border-amber hover:text-amber"
          >
